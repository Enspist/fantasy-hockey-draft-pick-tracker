...
class Settings(commands.Cog):
    ...
    async def rounds_set(
        ...
        # Seed any missing picks for all existing teams across all configured years
        ...
        for team in teams:
            await queries.seed_picks_for_team(
                self.bot.pool, guild_id, team["id"], years, number
            )

        await interaction.response.send_message(
            f"Draft set to **{number} rounds**. "
            f"All teams now have picks for rounds 1–{number} across {len(years)} year(s).",
            ephemeral=True,
        )
    ...
    async def years_set(
        ...
        # Seed any missing picks for all existing teams across the new year range
        settings = await queries.get_settings(self.bot.pool, guild_id)
        current_year = datetime.now().year
        ...
        for team in teams:
            await queries.seed_picks_for_team(
                self.bot.pool, guild_id, team["id"], years, settings["rounds"]
            )

        year_range = f"{current_year}–{current_year + number - 1}" if number > 1 else str(current_year)
        await interaction.response.send_message(
            f"Now tracking **{number} year(s)** of picks ({year_range}). "
            f"All teams have been seeded for the full range.",
            ephemeral=True,
        )
        await post_pick_board(self.bot, guild_id)
    ...
    async def years_show(self, interaction: discord.Interaction) -> None:
        ...
        year_range = (
            f"{current_year}–{current_year + years_ahead - 1}"
            if years_ahead > 1
